...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Graph(
                                    ...
                                    figure=px.pie(spacex_df, values='class', names='Launch Site', title='Total Successful Launches')
                                ),
                                ...
                                dcc.Graph(
                                    ...
                                    figure=px.scatter(
                                        ...
                                        title='Payload vs. Outcome by Booster Version Category'
                                    )
                                ),
...
def update_pie_chart(site):
    if site == 'ALL':
        ...
        fig = px.pie(filtered_df, names='class', title=f'Success vs. Failed Launches for {site}')
    return fig
# TASK 4:
...
def update_scatter_chart(site, payload_range):
    ...
    if site == 'ALL':
        title = 'Payload vs. Outcome by Booster Version Category (All Sites)'
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == site]
        title = f'Payload vs. Outcome by Booster Version Category for {site}'
...
# Which payload range(s) has the highest launch success rate? 5300 VAFB SLC-43
# Which payload range(s) has the lowest launch success rate? 362 CCAFS SLC-40
